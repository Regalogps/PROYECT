...
class Example(Frame):
    def __init__(self, master, index1, index2, *args, **kwargs):
        Frame.__init__(self, master, *args, kwargs)

 
        self.img = Label (self, image= index1)
        self.img .pack (fill= 'both', expand= True)
        self.img .bind ('<Configure>', lambda event, var= index2: self.resize(event,var))

    def resize(self, event, vara):

        self.image = vara .resize ((self.master .winfo_width(), self.master .winfo_height()))
...
class _Frame_boomer_IZQUIERDA (Frame, Interface):  #------------------------------ IZQUIERDA :  DELAY  /  MEDIR
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frame_img_DELAY = Example (self, self.master.master.Imageness [8], self.master.master.Imagenes [8],  bd=0)
        self.frame_img_DELAY .grid(column=0,row=0)
        #self.frame_img_DELAY .grid_propagate (0)

        self.frame_img_SIMULADOR = Example (self, self.master.master.Imageness [9], self.master.master.Imagenes [9], bd=0)  
...
class _Frame_boomer_DERECHA (Frame):  #------------------------------- DERECHA :  BASE  /  77
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lbl_BASE = Example (self, self.master.master.Imageness [10], self.master.master.Imagenes [10], bd=0)
        self.lbl_BASE . bind ("<Button-1>", self.ocultar)
        self.lbl_BASE . grid (column=0, row=0)
        self.lbl_BASE . grid_propagate(0)

        self.lbl_77 = Example (self, self.master.master.Imageness [11], self.master.master.Imagenes [11], bd=0)       
